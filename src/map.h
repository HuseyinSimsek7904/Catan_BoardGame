...
public:
  Map() {
    ...
    std::shuffle(tile.begin() + 1, tile.end(),
                 std::default_random_engine(seed));

    numbers = {0, 2, 3, 3, 4, 4, 5, 5, 6, 6, 8, 8, 9, 9, 10, 10, 11, 11, 12, 0};

    seed = std::chrono::system_clock::now().time_since_epoch().count();
    std::shuffle(numbers.begin() + 1, numbers.end(),
                 std::default_random_engine(seed));
  ...
  std::vector<int> getTrade() { return trade; }

  std::vector<std::vector<int>> getEdge() { return edge; }

  std::vector<int> getTile() { return tile; }

  std::vector<int> getNumbers() { return numbers; }

  Node getNode(int b) { return nodes[b]; }

  void setNode(Node n, int b) { nodes[b] = n; }
